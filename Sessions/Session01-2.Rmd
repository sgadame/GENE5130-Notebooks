...
```{r}
data %>%
  dim() #total entries and cols
```
```{r}
data %>%
  #head() #first 6 rows
  #tail() #last 6 rows
  glimpse() #summary of df structure and content
...
```{r}
subset <- data %>%
  select(gene, Samples, FPKM) ### Modify this code block by using the function select
...
size_not_cancer_abundant <- subset2 %>% dim()
...
brca_mean <- data %>% 
  filter(gene == 'BRCA1') %>%
  summarize(mean_expression = mean(FPKM))
```
...
data %>%
  filter(gene == 'BRCA1' | gene == 'BRCA2') %>%
  group_by(gene, characteristics_ch1, metastasis.ch1) %>%
  summarize(avg_expression = mean(FPKM))
```
...
```{r}
renamed <- data %>%
  rename(metastasis_status = metastasis.ch1,
         tissue_type = characteristics_ch1,
         sample_id = Samples) %>%
  select(-geo_accession, -organism_ch1) # Dropping unnecessary columns
```
...
high_expr_cancer_mets <- data %>%
  filter(FPKM > 500, 
         characteristics_ch1 == "tissue: breast tumor", 
         metastasis.ch1 == "yes")
```

2.- Top expressed genes: Find the tops 10 most highly expressed henes in normal breast tissue.
...
```{r} 
gene_set <- data %>%
  filter(gene %in% c('BRCA1', 'BRCA2', 'ALK', 'MYCN', 'TP53'))

ggplot(gene_set, aes(x=Samples, y=gene, fill=FPKM)) + 
  geom_tile() +
