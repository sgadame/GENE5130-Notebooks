...
```{r setup}
    ...
    knitr::opts_knit$set(root.dir = normalizePath("~/Downloads")) 
```
...
Removing genes with low counts is important for several reasons:
1. Genes with very low counts have high variance and low statistical power, making differential expression analysis less reliable.
2. Low count genes introduce noise into the multiple testing correction, potentially reducing our ability to detect truly differentially expressed genes.
3. They increase computational burden without adding meaningful biological information.
4. DESeq2's statistical model works best with genes that have sufficient read depth to provide accurate estimations.


#### Students that took R already, use base R to subset the  dds object based on the sum accross a row, 
say I just want to keep genes with > than 10 reads across all samples!?!

#### Students that haven;t take R think about a pseudocode way to do it!
```{r}
### WRITE YOUR CODE HERE
# Filter genes with at least 10 reads across all samples
...
**Answer Here**:
```{r}
# Count how many genes pass the filtering
sum(keep)
```
...
```{r}
# Set untreated (control) as the reference level
dds_filtered$dexamethasone <- relevel(dds_filtered$dexamethasone, ref = "untrt")
```
...
```{r}
### Chunk 2 - Normalized counts
head(counts(dds_filtered, normalized=TRUE))
```
...
- Based on the summary, we can see the number of upregulated and downregulated genes with adjusted p-value < 0.05.
